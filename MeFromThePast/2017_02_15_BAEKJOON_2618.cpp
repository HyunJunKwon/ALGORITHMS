...
void dfs(int nth)
{
	...
	if (check == true)
		return;
	if (result[nth].distance <= min(distanceInfo[1][nth], distanceInfo[2][nth]) && (result[nth].isVal == 'T' || result[nth].isVal == 'C'))
	{
		if (check == true)
			return;
		...
		if (check == true)
			return;
		if (nth >= 2 && (distanceInfo[1][nth] == distanceInfo[2][nth])) // 의심,,,? 계속 값이 같으면,,,???
		{
			if (result[nth].isVal != 'C')
			{
				result[nth - 1].isVal = 'F';
				dfs(nth - 1);
			}
	...
	if (check == true)
		return;
	if (result[nth].distance > min(distanceInfo[1][nth], distanceInfo[2][nth]))
	{
		if (distanceInfo[1][nth] > distanceInfo[2][nth])
		{
			result[nth].distance = distanceInfo[2][nth];
		}
		else
